...
export class ItemListService {
        ...
        return new Promise<IViewListResult>((resolved?: (value: IViewListResult) => void, rejected?: (reason: any) => void) => {
            ...
            if (ignoreCache) {
                ...
                this.getItemList(query, this.selectedList).then((items) => {
                    ...
                    delete this._cache.visibleList;
                    this.ensureVisibleListCache(searchField, regExp).then(() => {
                        loadViewList();
                    }).catch(rejected);
                }).catch(rejected);
            } else {
     ...
     */
    protected getVisibleList(items: IItemBase[], searchField?: string, regExp?: RegExp): Promise<IItemBase[]> {
        return new Promise<IItemBase[]>((resolved?: (result: IItemBase[]) => void, rejected?: (reason: any) => void) => {
            if (!items) {
                resolved([]);
                return;
                ...
                if (regExp) {
                    ...
                    let getFilteredList = (treeList: IItemBase[], depth: number, hidden: boolean) => {
                        ...
                        treeList && treeList.forEach((itm) => {
                            ...
                            if (itmTree.$items) {
                                odd = false;
                                let filteredChildren = getFilteredList(itmTree.$items, depth + 1, hidden || itmTree.collapsed || itm.visible === false);
                                if (filteredChildren) {
                                    if (!filteredItems) {
    ...
    private ensureSelectedItems(items: IItemBase[]) {
        ...
        let selectedList = items.filter((item) => this.selectedList.find((selected) => this.compareItems(selected, item)));
